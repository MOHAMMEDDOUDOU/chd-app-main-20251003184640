...
export default function ProductDetails() {
  ...
  const originalPrice = useMemo(() => {
    if (!product) return 0;
    const p = parseFloat(String(product.price || 0));
    return isNaN(p) ? 0 : p;
  }, [product]);

  const discountPrice = useMemo(() => {
    if (!product) return null as number | null;
    const d = product.discountPrice ?? (product as any).discount_price;
