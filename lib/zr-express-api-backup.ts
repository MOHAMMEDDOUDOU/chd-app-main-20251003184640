...
export class ZRExpressAPI {
  private static async makeRequest(endpoint: string, data: any): Promise<any> {
    try {
      // إعداد headers مع token و key في headers
      const headers: any = {
  ...
  static async createOrder(orderData: any): Promise<any> {
    try {
      ...
      // التحقق من نجاح إنشاء الطلبية
      if (response?.COUNT > 0 && response?.Colis && response.Colis.length > 0) {
        const createdOrder = response.Colis[0];
        return {
          success: true,
          message: "تم إنشاء الطلبية في ZR Express بنجاح",
          data: {
            Tracking: createdOrder.Tracking,
            id_Externe: createdOrder.id_Externe,
            MessageRetour: createdOrder.MessageRetour,
            Date_Creation: createdOrder.Date_Creation
          }
        }
      } else {
        return {
          success: false,
          message: "فشل في إنشاء الطلبية في ZR Express",
          data: response
        }
      }
    } catch (error) {
